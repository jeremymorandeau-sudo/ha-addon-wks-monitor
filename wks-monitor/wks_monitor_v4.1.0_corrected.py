#!/usr/bin/env python3
"""
WKS Monitor v4.1.0 - Version corrigée
- Correction parsing QPGS selon protocole Voltronic
- Correction appel parser QPIGS
- Correction parsing QPIWS complet
- Ajout champs manquants QPGSn
"""
...
class VoltronicParser:
    ...
    def parse_qpgs(resp: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(fields) > 26:
                data["battery_discharge_current_a"] = VoltronicParser.safe_int(fields[26])
...
def main():
    ...
    log(f"[BOOT] 🚀 WKS Monitor v4.1.0 - Polling {poll_interval}s")
    ...
    while True:
        ...
        for idx in range(inverter_count):
            ...
            # QPIGS - General Status (utilise le bon parser maintenant!)
            if enable_qpigs:
                resp = sr.query("QPIGS")
                if resp and len(resp) > 10:
                    qpigs_data = parser.parse_qpigs(resp)  # ✅ Correction: utilise parse_qpigs
